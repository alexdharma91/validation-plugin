...
(function ($) {
    ...
    var validationRules = {
        ...
        isNotZero: function(input){
            if($.isNumeric(input) && input != 0){return true;}
            else{return false;}
        },
    ...
    validateForm = function (form, settings) {
      ...
      if(!settings.beforeValidation.apply(form)){result = false;}

        form.find(":input").each(function (i, el) {
            var $this = $(this);
            if ($this.hasClass(settings.needValidateClass) && validateInput($this, settings) == false) {
                result = false;
            }
        });

       if(!settings.afterValidation.apply(form)){result = false;}
    ...
    /**
     * собираем валидаторы с инпута, пример data-validation="isNumeric;maxLength=20;"
     * @param jQuery(input)
    ...
    var methods = {
        init: function (options) {
            ...
            var settings = $.extend({
                commaToDot:false,
                needValidateClass: 'necessarily',
                noop: noop,
                beforeValidation: function(){ return true; },
                afterValidation: function(){ return true; }
            }, options);
            ...
            if ($(this).is(":input")) {
                if ($(this).hasClass(settings.needValidateClass)) {
